        ...
        for row in spamreader: #Schleife über die Liste
            if not row == []: #Ignoriert leere Spalten die in der csv leider aus "übersichtlichkeitsgründen" auftauchen
                ...
                if not link == "URL": #Okay bitte csv zukünftige mit dictwriter erstellen sonst werde ich sauer
                    ...
                    insertData(fixString(row[0]), fixString(row[1]), fixString(row[2]), fixString(row[3]), fixString(row[4]), fixString(description)) #Daten werden in die Datenbank eingefügt
...
def createCSV(namelist,cr_list,source_list,linklist,gesinnung):
    import csv
    import os
    file_name = 'Monsterliste.csv'
    with open(r'.\\' + file_name, mode='w') as monsterlist:
        writer = csv.DictWriter(monsterlist, fieldnames = ["Name", "CR", "Source","URL", "Gesinnung", "Beschreibung"])
        writer.writeheader()
        monsterlist = csv.writer(monsterlist, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)

        for x in range(len(namelist)):
            monsterlist.writerow([namelist[x].text, cr_list[x].text, source_list[x].get_attribute("title"),linklist[x].get_attribute("href"),gesinnung[x].text])  #bisher nur die beiden Attribute, bin schon am überlegen ob ich mir ne spider sparen kann :DD
            x += 1
...
def getAllButDescription():
    ...
    gesinnung = driver.find_elements_by_xpath("//span[@class='mon_wrp-size-type-align--token']")
    createCSV(namelist,cr_list,source_list,linklist,gesinnung)
    encoding()
    driver.close()
...
def addDescription():
    ...
    with open("Monsterliste_fixedEncoding.csv") as f_in, open("Monsterliste_withDescription.csv", 'w') as f_out:
        ...
        for x in spamreader:
            if not x == []:
                f_out.writerow([x[0], x[1], x[2], x[3], x[4], "1"])
